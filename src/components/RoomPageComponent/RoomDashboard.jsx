...
const RoomDashboard = ({ rooms, setRooms, currentUserId, onCreateClick }) => {
  ...
  const deleteRoom = async (roomId) => {
    ...
    try {
      ...
      if (data.success) {
        // ✅ Correct way to update the UI instantly
        setRooms((prevRooms) =>
      ...
      toast.error("Error Deleting Room: " + errorMsg);
    }
  };
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
        <div>
          ...
          <p className="text-gray-500 mt-2 font-medium">
            Active in{" "}
            <span className="text-blue-600">{rooms.length} workspaces</span>
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
        {rooms.map((room) => (
          <div
            key={room._id}
            className="flex flex-col h-full bg-white rounded-[2rem] border border-gray-100 p-6 sm:p-8 shadow-sm hover:shadow-xl transition-all group"
          >
            {/* <div className="flex justify-between items-start mb-6">
              ...
              {room.createdBy?._id === currentUserId && (
                <span className="border flex items-center gap-1.5 bg-green-50 text-green-700 px-3 py-1.5 rounded-xl text-[10px] uppercase tracking-widest font-black border border-green-100">
                  <ShieldCheck className="w-3 h-3" /> Admin
                  //Added the delet button
                </span>
              )}
            </div> */}
            <div className="flex justify-between items-start mb-6">
